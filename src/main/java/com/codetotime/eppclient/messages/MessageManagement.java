...
public class MessageManagement extends JFrame {
  ...
  private void refreshActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_refreshActionPerformed
    updateTableContent();
  ...
  private void messagesTableMouseDoubleClicked(java.awt.event.MouseEvent evt) {
    ...
    if (messagesTable.getSelectedRow() >= 0) {
      for (int i = 0; i < messageDetailWindows.size(); i++) {
        if (((MessageDetail) messageDetailWindows.get(i))
            .getMsgId()
  ...
  public void addMessage(Message message) {
    model.insertRow(
        ...
        new Object[] {
          message.getMsgId(),
          dateFormatter.format(message.getDateTime()),
          message.getTitle(),
          message.getRead(),
          message.getAck()
        });
