 ...
 * archive. It uses raw SELECT * and connects using the same configuration used by DAOs, so it works
...
public class DbExporter {
   ...
   * will contain contacts.csv, domains.csv and messages.csv.
   */
  public void exportAllToZip(File destinationZip) throws Exception {
    ...
    try (FileOutputStream fos = new FileOutputStream(destinationZip);
      ...
      // Contacts
      exportSingleTableToZipEntry(zos, "contacts.properties", "contacts.csv");

      // Domains
      exportSingleTableToZipEntry(zos, "domains.properties", "domains.csv");

      // Messages
      exportSingleTableToZipEntry(zos, "messages.properties", "messages.csv");
    }
  }
  ...
  private void exportSingleTableToZipEntry(
    ...
    if (schema == null || table == null) {
      throw new IllegalStateException(
          "Proprietà mancanti per l'esportazione: db.schema o db.table non definite in "
    ...
    // For Derby, DAOs use separate DB names: contacts/domains/messages. Infer from properties file
    // name.
    String dbNameForDerby = null;
    if (baseUrl != null
        && !baseUrl.toLowerCase().contains("mariadb")
        && !baseUrl.toLowerCase().contains("postgresql")) {
      dbNameForDerby = propertiesResourceName;
      if (dbNameForDerby != null && dbNameForDerby.endsWith(".properties")) {
        dbNameForDerby =
            dbNameForDerby.substring(0, dbNameForDerby.length() - ".properties".length());
      }
    }
  ...
  private static String buildJdbcUrl(String baseUrl, String dbName) {
    ...
    // Derby: append database name (contacts/domains/messages). If dbName is null, return baseUrl
    // as-is.
